...
export default async function PublicHomePage({
  ...
  const origins: string[] = Array.from(new Set(allRoutes.map((r: any) => (r.origin as string) || 'Dhaka Gabtoli')));
  if (origins.length === 0) origins.push('Dhaka Gabtoli', 'Chittagong GEC', 'Sylhet');

  const destinations: string[] = Array.from(new Set(allRoutes.map((r: any) => (r.destination as string) || 'Rajshahi University')));
  if (destinations.length === 0) destinations.push('Rajshahi University', 'Chittagong University', 'GST Cluster');

...
from app.models import (
    ...
    BusRoute, TripStop, Trip, FareRule, SeatLock,
...
async def seed_database():
    ...
    async with AsyncSessionLocal() as db:
        ...
        # 1. CREATE TENANT (SaaS Bus Company)
            ...
            seat_layout = SeatLayout(name="Standard", total_rows=10, total_cols=4, total_seats=40, layout_json="{}")
            db.add(seat_layout)
            await db.flush()
            seat_a1 = Seat(seat_number="A1", seat_layout_id=seat_layout.id, row_index=0, col_index=0, is_active=True, fare_zone_id=vip_zone.id)
            db.add(seat_a1)
            await db.flush()
            import collections
            seat_objects = collections.defaultdict(lambda: seat_a1)
            booking1 = Booking(
                tenant_id=tenant.id,

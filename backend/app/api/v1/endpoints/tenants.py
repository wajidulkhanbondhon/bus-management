...
async def list_tenants(db: WrappedAsyncSession = Depends(get_db)):
    return await db.query(Tenant).filter(Tenant.is_active == True).all()
...
async def create_tenant(
    ...
    existing = await db.query(Tenant).filter(Tenant.slug == req.slug).first()
    if existing:
